...
{
    var container = $('#main-chat-wrapper');
   container.animate({"scrollTop": $('#main-chat-wrapper')[0].scrollHeight}, "slow");
}
...
$(function()
{
    ...
    $("#loginBtn").on("click", function(e)
    {
        ...
        if($(".username-2").val().length < 2)
        {
            //ocultamos el mensaje de error
            $(".errorMsg").hide();
            //mostramos el mensaje de nuevo y ponemos el foco en el campo de texto
            $(".username-2").after("<div class='col-md-12 alert alert-danger errorMsg'>Debes introducir un nombre para acceder al chat.</div>").focus(); 
        ...
        manageSessions.set("login", $(".username-2").val());
    ...
    socket.on("userInUse", function()
    {
        ...
        $(".username-2").after("<div class='col-md-12 alert alert-danger errorMsg'>El usuario que intenta escoge está en uso.</div>").focus();
        return; 
    });
    ...
    socket.on("refreshChat", function(action, message)
    {
        ...
        if(action == "conectado")
        {
            $("#messages-wrapper").append("<p class='col-md-12 alert-info'>" + message + "</p>");
        }
        //si es una desconexión
        else if(action == "desconectado")
        {
            $("#messages-wrapper").append("<p class='col-md-12 alert-danger'>" + message + "</p>");
        }
        //si es un nuevo mensaje 
        else if(action == "msg")
        {
            $("#messages-wrapper").append('<div class="user-details-wrapper"><div class="col-md-12 user-details"><div class="bubble">'+ message +'</div></div></div>');
        }
        else if(action == "msg-yo")
        {
            $("#messages-wrapper").append('<div class="user-details-wrapper pull-right"><div class="user-details"><div class="bubble sender">'+ message +'</div></div></div>');  
        }
        //si el que ha conectado soy yo
        else if(action == "yo")
        {
            $("#messages-wrapper").append("<p class='col-md-12 alert-success'>" + message + "</p>");
        }
        //llamamos a la función que mantiene el scroll al fondo
        animateScroll();
    });
...
function renderForm()
{
    ...
    html += '<input type="text" id="username-2" class="form-control username-2" placeholder="Introduce un nombre de usuario">';
    html += '</div>';
